...
export function ImageCarousel({ images }: ImageCarouselProps) {
  ...
  useEffect(() => {
    ...
    const onSelect = () => {
      ...
      // Reset zoom state on slide change (just in case)
      setIsZoomed(false);
    };
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Only allow keyboard navigation if not zoomed (optional, but consistent with swipe)
      if (!isZoomed) {
  ...
  return (
    <>
      <div className="relative group w-screen ml-[calc(50%-50vw)]">
        ...
        {/* Embla Viewport */}
        <div className="overflow-hidden py-6" ref={mainEmblaRef}>
          {/* Embla Container */}
          <div className="flex touch-pan-y -ml-4"> 
            {images.map((src, index) => (
              <div key={index} className="flex-[0_0_auto] pl-4 min-w-0">
                <div 
                  className="relative w-[85vw] md:w-[1200px] h-[300px] md:h-[675px] rounded-[24px] overflow-hidden group/image cursor-pointer"
